...
# Test 3: 互补作用 9:7 比例检验 (df = 1)
cat("测试 3: 基因互补作用 9:7 比例检验 (香豌豆花色 O = 188, 140)...\n")
res_97 <- calculate_genetics_chisq(
  observed = c(188, 140),
  ratio = c(9, 7),
  df = 1
)
stopifnot(abs(res_97$chisq_calc - 0.15176) < 1e-3)
stopifnot(res_97$significance_level == "not_significant")
cat("  ✓ 测试 3 通过: 9:7 比例卡方 =", round(res_97$chisq_calc, 4), "P =", round(res_97$p_value, 4), "结论:", res_97$conclusion_short, "\n\n")

# Test 4: 显著差异数据 (0.01 < P <= 0.05)
cat("测试 4: 显著差异水平 (0.01 < P <= 0.05)...\n")
...
cat("  ✓ 测试 4 通过: chisq =", res3$chisq_calc, "结论:", res3$conclusion_short, "\n\n")

# Test 5: 极显著差异数据 (P <= 0.01)
cat("测试 5: 极显著差异水平 (P <= 0.01)...\n")
...
cat("  ✓ 测试 5 通过: chisq =", res4$chisq_calc, "结论:", res4$conclusion_short, "\n\n")

# Test 6: Yates 连续性校正 (df = 1)
cat("测试 6: Yates 连续性校正...\n")
...
cat("  ✓ 测试 6 通过: 未校正卡方=4.0, Yates校正后卡方=", res5$chisq_calc, "\n\n")
